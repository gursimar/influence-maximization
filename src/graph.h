...
class Graph
{
    public:
        ...
        void readVector(const vector <pair<int,int>> &edges, const vector<double> &prob, int n) {
            ...
            for(auto const &p: edges){
                ...
                hasnode[node1]=true;
                hasnode[node2]=true;
                add_edge(node1, node2, probi);
                //cout <<node1 << " " << node2 << " "<< probi << " "<< endl;
            }
        }



        Graph(string folder, string graph_file, const vector <pair<int,int>> &edges, const vector<double> &prob, int nodes):folder(folder), graph_file(graph_file){
            if (folder == "") {
                m=edges.size();
                n=nodes;
                visit_mark=vector<int>(n);
